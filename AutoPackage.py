        ...
        for line in lines:
            ...
            if match:
                ...
                if "en.lproj" in string_path:
                    pattern = re.compile(
                        r'nutstore', re.IGNORECASE)  # 替换“=”后半部分的名称
                    tarl = pattern.sub(Config.name_en, tarl)
                else:
                    pattern = re.compile("坚果云", re.IGNORECASE)
...
def replace_share():
    """替换 微信 和 QQ 的分享 Id"""
    path1 = os.path.join(
        _work_dir(), 'iphone-app/Nutstore/Classes/NTSApplicationDelegate.m')
    path2 = os.path.join(_work_dir(), 'iphone-app/Nutstore/Nutstore-Info.plist')
    path3 = os.path.join(_work_dir(
    ), 'iphone-app/Nutstore/Classes/FileOperationsUI/NTSShareFileViewController.m')

    wechat_paths = [path1, path2]
    for path in wechat_paths:
        _simple_replace(path, 'wx6e368f94b68c697f', Config.WxAppIdUrl)

    _simple_replace(path2, 'tencent1104634782', Config.QQAppIdUrl)

    qq_paths = [path1, path3]
    for path in qq_paths:
        _simple_replace(path1, '1104634782', Config.QQAppId)
...
def replace_email():
    """替换反馈意见邮箱"""
    path = os.path.join(
        _work_dir(), 'iphone-app/Nutstore/Classes/MoreLive/OtherSettingTableViewController.m')
    _simple_replace(path, 'ios.feedback@nutstore.net', Config.feedbackEmail)
...
def setup_appserver():
    ...
    server_conf_Path = os.path.join(
        _work_dir(), "iphone-app/Nutstore/Server-conf.plist")
...
def build_and_package():
    ...
    project_path = os.path.join(_work_dir(), 'iphone-app/Nutstore/')
    workspace_path = os.path.join(project_path, "Nutstore.xcworkspace")

    project = XcodeProject.load(
        os.path.join(project_path, "Nutstore.xcodeproj/project.pbxproj"))
    ...
    archive_cmd = 'xcodebuild -workspace %s -scheme %s -configuration %s -archivePath %s clean archive' % (
        workspace_path, "Nutstore", Config.configuration, archive_path)
